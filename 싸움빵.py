...
# TODO: arr의 각 요소를 set으로 변환하면 set은 중복처리하니까 총이 똑같은 게 있으면 하나로 퉁침 -> 한 놈이 가져가면 원래 하나 더 있어야 하는데 없어지는 거지
## 처리할 때 말 하나에 속해있는 거면 앵간하면 한 놈으로 묶어
...
def get_gun(idx,x,y): # 비교 후 큰 거는 갖고, 기존에 갖던 건 놓고 가는 로직이 잘 안 돼있음
